...
Go's `new()` function allocates memory, initializes it with the zero value of the specified type, and returns a pointer
to it:
...
The `new()` function is particularly useful when you need a pointer to a value but don't have an existing variable to
point to.

### Nil Pointers

When a pointer is declared but not initialized, it has the value `nil`, which represents the absence of a valid memory
address:
...
This works because Go performs escape analysis and allocates variables on the heap when they need to survive after the
function returns.
...
In some cases, you might need a pointer to a pointer (double pointer), usually when you need to modify the pointer
itself:
...
Unlike slices, maps in Go are reference types, so you don't usually need pointers to maps unless you want to modify the
map variable itself:
...
Even with garbage collection, pointers can still cause memory leaks if you keep references to objects you no longer
need:
...
Go provides the `unsafe` package for advanced, low-level memory operations. This is rarely needed and should be used
...
In most languages, using a pointer to memory that has been freed can cause dangling pointer bugs. Go's garbage collector
helps prevent this, but issues can still arise in specific scenarios:
...
Implement a stack data structure using pointers. A stack is a Last-In-First-Out (LIFO) data structure where elements are
...
Implement a generic swap function using pointers. This exercise demonstrates how pointers allow you to modify variables
passed to functions.
...
Implement a simple binary search tree (BST) using pointers. A binary search tree is a hierarchical data structure where
each node has at most two children, with values less than the node to the left and values greater than the node to the
right.
